  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (process.platform === 'linux' && headless === false) {
        console.log(
          clc.yellow(
            '[WARNING] [PUPPETEER-REAL-BROWSER] | On the Linux platform you can only run the browser in headless true mode.'
          )
        )
        headless = true
      }
      ...
      const chromeFlags = ['--no-sandbox', ...args]

      if (headless === true && process.platform !== 'linux') {
        chromeFlags.push('--headless=new')
      }

      if (proxy && proxy.host && proxy.host.lasyncength > 0) {
      ...
      if (process.platform === 'linux') {
        try {
          ...
          console.log(
            clc.red(
              '[ERROR] [PUPPETEER-REAL-BROWSER] | You are running on a Linux platform but do not have xvfb installed. The browser can be captured. Please install it with the following command\n\nsudo apt-get install xvfb'
            )
          )
